...
class BounceImage {
  ...
  public void step(long deltaT) {
	...
	x = (float) (x + Vx * deltaT + (Ax / 2.0) * deltaT * deltaT);
	y = (float) (y + Vy * deltaT + (Ay / 2.0) * deltaT * deltaT);
	if (x <= 0.0f) {
	  x = 0.0f;
	...
	findex = findex + Vr * deltaT;
	if (collision_x || collision_y) {
	  Vr = Vr * Ar;
	}
	
	while (findex <= 0.0) {
	  findex = findex+parent.bounceimages.length;
	}
